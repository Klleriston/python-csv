...
# Colunas exatas das contas a pagar
COLUNAS_TEMPLATE = [
...
def formatar_coluna(valor, coluna):
    ...
    if pd.isna(valor) or valor == '':
        ...
        if coluna in ['Valor documento', 'Saldo']:
            return 0
        else:
            return ''
...
def dividir_contas_pagar():
    """
    Divide a planilha de contas a pagar em partes menores de até 500KB,
    utilizando divisão direta por número de linhas.
    """
    print(f"Dividindo planilha de contas a pagar em partes de até {MAX_FILE_SIZE/1024:.0f}KB")
    ...
    for i in range(total_arquivos):
        ...
        nome_arquivo = f"contas_pagar_parte_{i+1:03d}.xlsx"  # Usar 3 dígitos para comportar mais arquivos
